...
| 2 | Real 60-line RFQ | **0** | — | **ABSENT — see below** |
...
### Component 2 is missing, and I am not going to substitute for it

There is no 60-line real RFQ in either repository. Production holds **one** RFQ with **two**
lines. The only RFQ-shaped fixture is a single-line cement draft in the API repo. Every
other candidate — `site-safety-02`, `datacenter-cyber-01`, `warehouse-ops-02`,
`large-boq-480-lines.csv`, `booklet-02-extra` — declares itself synthetic on its first page
(«وثيقة اختبار آلي», «بيانات افتراضية غير رسمية», `FARQ-TEST-*`). Filling this slot with a
synthetic booklet would reintroduce exactly the failure the Gold Set exists to prevent.

**The slot is reserved and the harness already counts it.** The owner needs to supply a real
RFQ he actually sent, or a real tender booklet, and it drops in.
