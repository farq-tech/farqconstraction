...
type Scored = {
  ...
  shortfall: string[]
  /**
   * NAMED AN INTENT WHERE THE FAMILY WAS THE COMPLETE ANSWER.
   *
   * Reported separately because the existing confident-wrong definition cannot
   * see it — it only compares intents when the adjudication names one — and the
   * first shadow run produced 25 of these and looked like a triumph. It is not
   * a wrong trade, so no RFQ goes to the wrong supplier; it is a wrong SPLIT,
   * which fragments a supplier pool that should have stayed whole. That is a
   * real harm with its own name, and it belongs in the report rather than
   * inside a headline coverage number.
   */
  overSpecified: string[]
}
...
function score(
  ...
  const out: Scored = { lines: rows.length, intent: 0, poolable: 0, unknown: 0, complete: 0, abstentions: 0, confidentWrong: [], shortfall: [], overSpecified: [] }
  for (const [i, [line, wantFamily, wantIntent]] of rows.entries()) {
    ...
    if (a.intent !== null && wantIntent === null && wantFamily !== null && a.family === wantFamily) {
      out.overSpecified.push(`${a.family}/${a.intent}  the family was the complete answer  ::  ${line}`)
    }
  }
  return out
}
...
function report(label: string, rows: Adjudicated[], run: Awaited<ReturnType<typeof runCorpus>>) {
  ...
  row('over-specified split', v10.overSpecified.length, cascade.overSpecified.length)
  ...
  const verdict =
    cascade.confidentWrong.length > v10.confidentWrong.length
      ...
      : cascade.complete < v10.complete
        ? 'FAILURE — completeness to the reachable ceiling fell.'
        : cascade.poolable > v10.poolable || cascade.complete > v10.complete
          ? 'GAIN — coverage up, confident-wrong not up.'
          : 'NO CHANGE — nothing gained, nothing lost.'
  console.log(`\n  VERDICT: ${verdict}`)

  if (cascade.overSpecified.length) {
    console.log(`\n  --- cascade over-specified (${cascade.overSpecified.length}) — right trade, invented depth ---`)
    for (const w of cascade.overSpecified) console.log(`    ${w}`)
  }
  ...
  for (const r of disagreements) {
    ...
    if (r.candidates[0]?.evidence.length) {
      console.log(`      evidence    : ${r.candidates[0].evidence.join(', ')}${r.candidates[0].discriminating_evidence.length ? `   [discriminating: ${r.candidates[0].discriminating_evidence.join(', ')}]` : '   [no discriminating term]'}`)
    }
    // Ownership refusals are the mechanism working and there are dozens per
    // line; only a real guard, veto or conflict refusal is worth a human's eye.
    const realRefusals = r.refused.filter((x) => x.refusals.some((f) => f.rule !== 'family_ownership'))
    if (realRefusals.length) {
      console.log(
        `      guard-refused: ${realRefusals
          .map((x) => `${x.intent_id} (${x.refusals.filter((f) => f.rule !== 'family_ownership').map((f) => `${f.rule}: ${f.detail}`).join('; ')})`)
          .join(' | ')}`,
      )
    }
  }
