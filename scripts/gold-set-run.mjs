 ...
 *
 *   node scripts/gold-set-run.mjs [--no-heldout] [--version=v10]
 *
 * `--version` pins the resolver to a reconstructed generation under
 * scripts/.tmp-vers/, instead of the working tree. This matters more than it
 * sounds: the working tree moved from v10 to v11 mid-session, and only the
 * recorded fingerprint caught it. Never trust the tree to be the version you
...
const PIN = (process.argv.find((a) => a.startsWith('--version=')) || '').split('=')[1] || null
const MODULE_PATH = PIN ? `/scripts/.tmp-vers/${PIN}/procurementOntology.ts` : '/src/lib/procurementOntology.ts'
const DATA_PATH = PIN ? `scripts/.tmp-vers/${PIN}/procurementOntology.data.json` : 'src/lib/procurementOntology.data.json'
const SRC_PATH = PIN ? `scripts/.tmp-vers/${PIN}/procurementOntology.ts` : 'src/lib/procurementOntology.ts'
const TOP_N = 5

// ---------------------------------------------------------------- fingerprints
const sha16 = (p) => createHash('sha256').update(readFileSync(p)).digest('hex').slice(0, 16)
const fingerprint = {
  payload_sha16: sha16(DATA_PATH),
  resolver_sha16: sha16(SRC_PATH),
  ontology_version: JSON.parse(readFileSync(DATA_PATH, 'utf8')).version,
  pinned: PIN ? `scripts/.tmp-vers/${PIN}` : 'working tree (may move under you)',
...
/*
 * Component 2. The owner's real 60-line RFQ lives in the PAUSED `farq` Supabase
 * project and was not reachable; a concurrent lane assembled this named
 * substitute from the same real catalogue, and labels itself a substitute
 * everywhere. Carried as component 2 with that status attached rather than left
 * empty, because 60 real short-form lines are worth more than a reserved slot.
 * It is NOT held out — another lane has already adjudicated it.
 */
const rfqPath = 'fixtures/boq/catalogue-shortform-60.adjudicated.json'
if (existsSync(rfqPath)) {
  const src = JSON.parse(readFileSync(rfqPath, 'utf8'))
  for (const [i, row] of src.rows.entries()) {
    items.push({ id: `RFQ-${String(i + 1).padStart(2, '0')}`, component: 'rfq_60_substitute', line: row[0], adjudicated_family: row[1] ?? null })
  }
}
...
const m = await server.ssrLoadModule(MODULE_PATH)
...
const components = ['booklet_68', 'rfq_60_substitute', 'error_corpus', 'fresh_heldout']
const report = {
  instrument: 'gold_set',
  ...
  composition: {
    ...
    rfq_60_substitute: items.filter((i) => i.component === 'rfq_60_substitute').length,
    rfq_60_status: 'SUBSTITUTE — the owner\'s real 60-line RFQ lives in the PAUSED `farq` Supabase project and was not reachable. fixtures/boq/catalogue-shortform-60.adjudicated.json holds 60 real short-form catalogue lines and is named a substitute wherever reported. NOT held out.',
...
const stem = `fixtures/gold/baseline-${fingerprint.ontology_version}`
writeFileSync(`${stem}.report.json`, JSON.stringify(report, null, 1))
writeFileSync(`${stem}.rows.jsonl`, rows.map((r) => JSON.stringify(redact(r))).join('\n') + '\n')
const vaultRows = join(homedir(), '.farq-gold-heldout', `baseline-${fingerprint.ontology_version}.rows.unredacted.jsonl`)
...
console.log(`\nwritten: ${stem}.report.json (+ .rows.jsonl, held-out lines redacted to HMAC)`)
